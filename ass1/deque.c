...
void recursive_reverse_node(Node *node);
...
void iterative_reverse(Deque *deque) {
  ...
  // Swap the top and bottom of deque
  temp = deque->bottom;
  deque->bottom = deque->top;
  deque->top = temp;

  // Swap prev and next for nodes in the deque
  while (curr != NULL) {
    reverse_node(curr);
    curr = curr->prev;
  }
...
void recursive_reverse(Deque *deque) {
  recursive_reverse_node(deque->top);
...
void split_deque(Deque *deque, int k) {
  Deque *front = new_deque();
  Deque *back = new_deque();
  int data;

  while (deque->size != 0) {
    data = deque_pop(deque);
    // put smaller data at the back and bigger in front
    if (data < k) {
      deque_insert(back, data);
    } else {
      deque_insert(front, data);
    }
  }
  ...
  *deque = *front;
  while (back->size != 0) {
    deque_insert(deque, deque_pop(back));
  }
}

// TODO: Add any other functions you might need for your Deque module

// Swap the prev and next nodes of the node
void reverse_node(Node *node) {
  assert(node != NULL);
  Node *temp = node->next;
  node->next = node->prev;
...
// Swap the prev and next nodes of a deque recursively
void recursive_reverse_node(Node *node) {
  assert(node != NULL);
  reverse_node(node);
  if (node->prev != NULL) {
    recursive_reverse_node(node->prev);
  }
}